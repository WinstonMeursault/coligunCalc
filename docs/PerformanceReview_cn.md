...
**现象：** 单级和多级导数评估分别调用互感与梯度 coil-level API；两者都完整遍历 `n_nodes^4` 积分点并重复计算椭圆积分。位置：`src/simulation/single_stage_sim.cpp:153-162`、`src/simulation/multi_stage_sim.cpp:248-259`、`src/physics/mutual_inductance.cpp:184-222`。

**依据：** Ousterhout — Information Hiding；Code Complete — avoid redundant computation。

**后果：** `gprofng` 单级测试采样中 `ellint_1` 独占 54.65% CPU、`ellint_2` 独占 17.80%；两个 coil-level 函数包含时间分别为 40.72% 和 43.87%。

**建议：** 增加 fused kernel，一次积分同时得到 `M` 和 `dM/dz`，共享几何映射、`K/E` 和中间量；以数值回归和同机基准验证收益。

### CPU-02 [P1] 缓存 Gauss-Legendre 规则，避免热循环分配

**现象：** `integrate_4d` 每次调用 `gauss_legendre`，后者按值返回带两个 `std::vector<double>` 的结构。位置：`src/physics/mutual_inductance.cpp:144-145`、`src/physics/quadrature.cpp:17-177`。

**依据：** Fowler — Repeated Work；McConnell — Data Structure and Allocation Discipline。

**后果：** 每个 stage/filament 的每个导数评估都会重复构造节点与权重存储，产生不必要的小对象分配和初始化；尚需分配计数验证其占比。

**建议：** 使用进程内不可变静态数组和 `std::span`/固定容量视图；验证数值、线程安全和分配次数。

### CPU-03 [P2] 合并或自适应 OpenMP 并行区

**现象：** OpenMP `parallel for` 位于每个 stage 的导数计算内，每步/RK4 子步重复 fork/join。位置：`src/simulation/single_stage_sim.cpp:145`、`src/simulation/multi_stage_sim.cpp:230-234`。

**依据：** Hunt & Thomas — Orthogonality；Meszaros — Slow Tests/feedback loop。

**后果：** `gprofng` 中 `GOMP_parallel` 包含时间约占 4.33% CPU；低任务量时线程管理可能抵消并行收益。

**建议：** 展平 active stage/filament 任务或复用外层并行区，并按任务量自适应选择串行/并行；覆盖多 stage、filament 和线程数基准。

### CPU-04 [P2] 减少每步完整积分状态复制

**现象：** Euler 的 `step` 和 `make_trial` 连续复制 `IntegrationState`，其中 Eigen 向量和多态 excitation snapshot 重新分配/克隆。位置：`src/simulation/*_stage_sim.cpp` 的 `step`/`make_trial` 和 `src/simulation/integration_state.cpp:35-45`。

**依据：** Fowler — Repeated Work；Ousterhout — Strategic vs. Tactical Programming。

**后果：** 额外内存写入、堆分配和 cache 压力会随 filament 数和步数增长；当前 profile 说明它不是主热点，但仍是可测的次级优化点。

**建议：** 引入预分配 trial state；无事件 Euler 路径直接写入下一状态，RK4 复用存储，事件定位时才复制，并以 allocation/RSS/数值回归验证。
