...
def calcL(ri: float, re: float, l: float, Nc: float, limit: int = 200) -> float:
    ...
    def integrationT(x: float) -> float:
        return np.power(U(x), 2) * (q * x + np.exp(-1 * q * x) - 1)

    T = quad(integrationT, 0, np.inf, limit=limit)[0]

    return 2 * np.pi * μ0 * Nc ** 2 * ri ** 5 * T

def calcm(Ra: float, Rb: float, d: float) -> float:
    m = (4 * Ra * Rb) / ((Ra + Rb)**2 + d**2)

    return np.clip(m, 1e-12, 1 - 1e-10)
...
def calcM_CF(Ra: float, Rb: float, d: float) -> float:
    ...
    return μ0 * np.sqrt(Ra * Rb) * ((2 / k - k) * eK(m) - (2 / k) * eE(m))
...
def calcM(Rai: float, Rae: float, La: float, Na: int, Rbi: float, Rbe: float, Lb: float, Nb: int, d: float) -> float:
    ...
    def integrationM(r1, z1, r2, z2):
        ra = (Rae + Rai)/2 + (Rae - Rai)/2 * r1
        rb = (Rbe + Rbi)/2 + (Rbe - Rbi)/2 * r2
        za = z1 * La / 2
        zb = z2 * Lb / 2
        dz = abs(zb - za)

        return calcM_CF(ra, rb, dz)

    # Jacobian
    J = ((Rae - Rai)/2) * ((Rbe - Rbi)/2) * (La/2) * (Lb/2)

    return Na * Nb * J * nquad(integrationM, [[-1,1],[-1,1],[-1,1],[-1,1]], opts={"epsabs":1e-6,"epsrel":1e-6})[0]
...
def calcdM_CF(Ra: float, Rb: float, d: float) -> float:
    ...
    return (μ0 * np.sqrt(m) * d * (2 * (1 - m) * eK(m) - (2 - m) * eE(m))) / (4 * (1 - m) * np.sqrt(Ra * Rb))
...
def calcdM(Rai: float, Rae: float, La: float, Na: int, Rbi: float, Rbe: float, Lb: float, Nb: int, d: float) -> float:
    ...
    def integrationdM(r1: float, z1: float, r2: float, z2: float) -> float:
        ...
        return calcdM_CF(ra, rb, dz)
    
    # Jacobian
    J = ((Rae - Rai)/2) * ((Rbe - Rbi)/2) * (La/2) * (Lb/2)
    
    return Na * Nb * J * nquad(integrationdM, [[-1, 1], [-1, 1], [-1, 1], [-1, 1]], opts={"epsabs":1e-6,"epsrel":1e-6})[0]
